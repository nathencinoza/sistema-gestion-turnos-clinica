 ...
 * *****************************************************************/
void abb_iterar(nodo_abb_t* nodo, bool visitar(const char*, void*, void *, size_t*), void *extra_empezar, void* extra_terminar, bool* stop, void* contador) {
    if (!nodo || !(*stop)) {
        return;
    }
    abb_iterar(nodo->izq, visitar, extra_empezar, extra_terminar, stop, contador);
    if (strcmp(nodo->clave, extra_empezar) >=0) {
        if (!(*stop) || !(visitar(nodo->clave, nodo->dato, extra_terminar, contador))) {
            *stop = false;
            return;
        }
    }
    abb_iterar(nodo->der, visitar, extra_empezar, extra_terminar, stop, contador);
}
void abb_in_order(abb_t *arbol, bool visitar(const char *, void  *, void *, size_t*), void *extra_empezar, void *extra_terminar, size_t* contador) {
    bool stop = true;
    abb_iterar(arbol->raiz, visitar,extra_empezar, extra_terminar, &stop, contador);
}
/* *****************************************************************
